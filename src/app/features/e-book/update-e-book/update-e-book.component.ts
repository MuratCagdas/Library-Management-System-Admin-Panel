...
export class UpdateEBookComponent implements OnInit {
  ...
  createForm(data: IEBook) {
    this.updateEBookForm = this.fb.group({
      ...
      isbnCode: [data.isbnCode, [Validators.required]],
      eBookTitle: [data.eBookTitle, [Validators.required]],
      authorName: [data.authorName, [Validators.required]],
      fileUrl: [data.imageUrl, [Validators.required]],
      file: [data.fileUrl, [Validators.required]],
      releaseDate: [data.releaseDate, [Validators.required]],
      pageCount: [data.pageCount, [Validators.required]],
      ...
      this.crudservice
        ...
        .pipe(
          catchError((err: HttpErrorResponse) => {
            return throwError(() => {
              this.alertify.message(err.error, {
  ...
  uploadImage(data: any) {
    ...
    this.updateEBookForm.patchValue({ fileImage: image });
  }
  uploadPDF(data: any) {
    const pdf = data.target.files[0];
    this.updateEBookForm.patchValue({ file: pdf });
  }
}
