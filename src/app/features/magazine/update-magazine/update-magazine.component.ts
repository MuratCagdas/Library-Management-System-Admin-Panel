...
export class UpdateMagazineComponent implements OnInit {
  ...
  createForm(data: IMagazine) {
    this.updateMagazineForm = this.fb.group({
      ...
      issnCode: [data.issnCode, [Validators.required]],
      magazineTitle: [data.magazineTitle, [Validators.required]],
      number: [data.number, [Validators.required]],
      releaseDate: [
        this.datepipe.transform(new Date(data.releaseDate), 'yyyy-MM-dd'),
        [Validators.required],
      ],
      publisherId: [data.publisherId, [Validators.required]],
  ...
  UpdateMagazineSubmit(data: any) {
    if (this.updateMagazineForm.valid) {
      this.crudservice
        .put({ controller: this.controller }, data)
        .pipe(
          catchError((err: HttpErrorResponse) => {
            return throwError(() => {
              this.alertify.message(err.error.Title, {
