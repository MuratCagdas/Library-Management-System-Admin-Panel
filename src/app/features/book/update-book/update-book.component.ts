...
import {
  ...
  LocationListConst,
  publisherListConst,
} from '../../../shared/constants/SearchTypeList';
...
import {
  ...
  MessageType,
  Position,
} from '../../../core/services/alertify/alertify.service';
...
export class UpdateBookComponent implements OnInit {
  ...
  createForm(book: IBook) {
    ...
    this.updateBookForm = this.fb.group({
      ...
      isbnCode: [book.isbnCode, [Validators.required]],
      locationId: [book.location?.id, [Validators.required]],
      bookTitle: [book.bookTitle, [Validators.required]],
      bookEdition: [book.bookEdition, [Validators.required]],
      releaseDate: [book.releaseDate, [Validators.required]],
      pageCount: [book.pageCount, [Validators.required]],
      publisherId: [book.publisherId, [Validators.required]],
      categoryId: [book.categoryId, [Validators.required]],
      file: [book.imageUrl, [Validators.required]],
    });
  }
  ...
  UpdateBookSubmit(data: IBook) {
    ...
    if (this.updateBookForm.valid) {
      this.crudservice
        .put({ controller: this.controller }, formData)
        .pipe(
          catchError((err: HttpErrorResponse) => {
            return throwError(() => {
              this.alertify.message(err.error.mesage, {
        ...
        .subscribe((data) => {
          this.alertify.message('Success Added', {
      ...
      this.alertify.message('Please Fill All Blanks', {
        messageType: MessageType.Error,
        position: Position.TopCenter,
        delay: 2,
      });
  ...
  uploadImage(data: any) {
    ...
    this.updateBookForm.patchValue({ file: image });
  }
