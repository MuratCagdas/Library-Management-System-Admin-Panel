...
import {
  EAnnoucementTag,
  IAnnoucement,
} from '../../../shared/models/annoucement';
import {
  AlertifyService,
  MessageType,
  Position,
...
export class UpdateAnnouncementComponent implements OnInit {
  ...
  announceTags = Object.values(EAnnoucementTag).filter(
    (key) => typeof key === 'string'
  );
  announceTagsValue = Object.values(EAnnoucementTag).filter(
  ...
  createForm(data: IAnnoucement) {
    this.updateAnnouncementForm = this.fb.group({
      ...
      file: [data.imageUrl, [Validators.required]],
    });
  }
  ...
  uploadImage(data: any) {
    ...
    this.updateAnnouncementForm.patchValue({ file: image });
  }
  TextAreaValue() {
    ...
    this.updateAnnouncementForm.patchValue({ content: value });
  }
}
