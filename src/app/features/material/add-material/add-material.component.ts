...
export class AddMaterialComponent implements OnInit {
  ...
  materialTags = Object.values(EMaterialType).filter(
    (key) => typeof key === 'string'
  );
  materialTagsValue = Object.values(EMaterialType).filter(
  ...
  ngOnInit(): void {
    this.addMaterialForm = this.fb.group({
      ...
      materialType: [1, [Validators.required]],
  ...
  addMaterialSubmit(data: any) {
    ...
    if (this.addMaterialForm.valid) {
      this.crudService
        .post<IMaterial>({ controller: this.ControllerMaterial }, data)
        .pipe(
