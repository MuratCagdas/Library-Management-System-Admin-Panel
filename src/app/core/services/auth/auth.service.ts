...
import {
  ...
  MessageType,
  Position,
} from '../alertify/alertify.service';
...
export class AuthService {
  ...
  private revokeToken(): Observable<RevokeTokenResponse> {
    return this.crudservice.revokeToken<null, RevokeTokenResponse>(
      { controller: 'Auth', action: 'RevokeToken' },
      null
    );
  }
  ...
  public getStaffFromAuth(): Observable<LibraryResponse | null> {
    return this.getUserFromAuth().pipe(
      switchMap((user) => {
        if (user) {
          return this.crudservice.get<LibraryResponse>({
            controller: 'LibraryStaffs',
            action: `${user.libraryStaffId}`,
          });
        }
        return of(null);
      })
  ...
  public getUserFromAuth(): Observable<UserResponse | null> {
    return this.getAccessToken().pipe(
      switchMap((accessToken) => {
        ...
        return this.crudservice.get<UserResponse>({
          controller: 'Users',
          action: 'GetFromAuth',
        });
      })
